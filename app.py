...
.similarity-bar  { height: 4px; background: #1F2937; border-radius: 2px; margin-top: 4px; }
.similarity-fill { height: 4px; background: #22C55E; border-radius: 2px; }
...
.off-banner {
    ...
    border: 1px solid #059669; color: #6EE7B7;
    border-radius: 8px; padding: 7px 14px;
    font-size: 0.8rem; font-weight: 600; margin-bottom: 12px;
}
.adv-banner {
...
/* ── Divider ── */
.section-divider { border: none; border-top: 1px solid #1F2937; margin: 1.5rem 0; }
...
.stButton > button {
    background: #22C55E !important; color: #0D0F14 !important;
...
.stTabs [data-baseweb="tab-list"] {
    background: #1A1D24; border-radius: 10px; padding: 4px; gap: 4px;
}
.stTabs [data-baseweb="tab"] {
    background: transparent; border-radius: 7px; color: #9CA3AF;
    font-family: 'Space Grotesk', sans-serif; font-weight: 600; font-size: 0.85rem;
}
.stTabs [aria-selected="true"] { background: #22C55E !important; color: #0D0F14 !important; }
...
def fetch_from_off(barcode: str) -> dict | None:
    ...
    try:
        ...
        off_cat_str = p.get("categories_hierarchy", []) or p.get("categories_tags", []) or []
...
def compare_ingredients_sim(ing1: str, ing2: str) -> dict:
    """Similarité textuelle entre deux listes d'ingrédients."""
    ...
    return {"score": pct, "confidence": confidence,
            "detail": f"Similarité des ingrédients : {pct}% ({confidence})"}
...
def analyze_packaging_with_claude(image1_bytes: bytes, image2_bytes: bytes) -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            '{"meme_usine_probable": true, '
            '"niveau_confiance": "haute|moyenne|faible|indéterminée", '
            '"indices_trouves": ["indice 1", "indice 2"], '
...
def render_advanced_comparison(df: pd.DataFrame) -> None:
    """Section de comparaison avancée : photo IA + estampille + ingrédients + lieu."""
    st.markdown(
        '<span class="adv-banner">🔬 Combine 4 méthodes pour identifier la même usine sans code EMB</span>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.markdown(
            '<div style="color:#22C55E;font-size:0.75rem;font-weight:600;'
            'text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">PRODUIT A</div>',
            unsafe_allow_html=True,
        )
        ...
        if query_a:
            ...
            if not matches.empty:
                labels_a = matches.apply(
                    lambda r: f"{safe_str(r['marque'])} — {safe_str(r['nom'])}", axis=1
                ).tolist()
                choice_a = st.selectbox("Sélectionner A", labels_a, key="adv_sel_a")
                prod_a = matches.iloc[labels_a.index(choice_a)]
 
    with col2:
        st.markdown(
            '<div style="color:#22C55E;font-size:0.75rem;font-weight:600;'
            'text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">PRODUIT B</div>',
            unsafe_allow_html=True,
        )
        ...
        if query_b:
            ...
            matches = df[mask]
            if not matches.empty:
                labels_b = matches.apply(
                    lambda r: f"{safe_str(r['marque'])} — {safe_str(r['nom'])}", axis=1
                ).tolist()
                choice_b = st.selectbox("Sélectionner B", labels_b, key="adv_sel_b")
                prod_b = matches.iloc[labels_b.index(choice_b)]
    ...
    # ── Méthode 2 : Analyse visuelle IA (Claude Vision) ─────────────────────
                    ...
                    if est1 and est1 == est2:
                        ...
                        if off_products:
                            st.markdown(
                                ...
                                'Autres produits de cette usine sur Open Food Facts :</div>',
                                unsafe_allow_html=True,
                            )
                            st.dataframe(pd.DataFrame(off_products), use_container_width=True, hide_index=True)
        else:
    ...
    if total == 0:
        st.info("ℹ️ Aucune méthode applicable. Ajoutez des photos ou sélectionnez des produits.")
    else:
        if votes_oui > votes_non:
            verdict_color, verdict_icon, verdict_txt = "#22C55E", "✅", "Même usine probable"
        elif votes_non > votes_oui:
            verdict_color, verdict_icon, verdict_txt = "#F87171", "❌", "Usines probablement différentes"
        else:
            verdict_color, verdict_icon, verdict_txt = "#F59E0B", "❓", "Résultat ambigu"
        ...
        st.markdown(
            ...
            f'{votes_oui} méthode(s) convergent · {votes_non} divergent(s) · sur {total} appliquée(s)'
            f'</div></div>',
            unsafe_allow_html=True,
        )
...
def find_clones(df: pd.DataFrame, product: pd.Series, search_code: str) -> pd.DataFrame:
    ...
    Stratégie par priorité :
      1. Même catégorie + même usine (EMB)
      2. Même catégorie seule
      3. Même usine + nom similaire (fallback si catégorie vide)
    Tri final : similarité ingrédients + bonus +10 pts si même EMB.
    """
...
def display_product(p: pd.Series) -> None:
    ...
    with col2:
        ...
        cat_badge = f'<span class="badge badge-cat">📂 {categorie}</span>' if categorie else ""
        st.markdown(
    ...
    for _, c in clones.head(10).iterrows():
        ...
        with st.expander(f"✅ {label}  •  {pct}% de ressemblance"):
            ...
            with cb:
                ...
                st.markdown(
                    f'<p style="font-size:0.82rem;color:#9CA3AF;line-height:1.5">'
                    f'{ing[:280]}{"…" if len(ing)>280 else ""}</p>',
                    unsafe_allow_html=True,
                )
...
def display_add_form(barcode: str, df: pd.DataFrame) -> None:
    ...
    # 2. Analyse IA par photo
    with st.expander("📷 Compléter via photo d'étiquette (IA)", expanded=not bool(off_data)):
        st.caption("Prenez en photo l'étiquette. L'IA (Gemini) tentera d'extraire les informations manquantes.")
    ...
    if ai_fill:
        st.markdown('<span class="ai-banner">✨ Champs complétés par l\'IA — vérifiez</span>', unsafe_allow_html=True)
    ...
    with c1:
        ...
        new_emb    = st.text_input(
            "Code usine EMB (optionnel)",
            value=defaults["emb"],
            help="Format : FR 12.345.678 CE — laissez vide si absent sur l'emballage",
        )
    ...
    with c2:
        ...
        new_cat = st.selectbox(
            "Catégorie *", options=cat_options, index=cat_index,
            help="Sert à trouver des alternatives comparables.",
        )
        if new_cat in ("", "Autres"):
            new_cat_libre = st.text_input(
                "Catégorie personnalisée (optionnel)",
                value=cat_default if cat_default not in CATEGORIES[:-1] else "",
            )
    ...
    if st.button("💾 Enregistrer dans la base"):
        ...
        # EMB volontairement optionnel — la comparaison avancée fonctionne sans
        ...
        if errors:
            ...
            new_line = {
                ...
                "sucre": new_sucre, "sel": new_sel, "energie_100g": new_kcal,
                "image_url": new_img.strip(), "usine_lieu": new_lieu.strip(),
...
def main() -> None:
    ...
    if not barcode:
        ...
        st.markdown('<hr style="border:none;border-top:1px solid #1F2937;margin:0.5rem 0 1.5rem">', unsafe_allow_html=True)
        with st.expander("🔬 Comparaison avancée — comparer deux produits sans code-barres", expanded=False):
    ...
    st.markdown('<hr style="border:none;border-top:1px solid #1F2937;margin:2rem 0 1rem">', unsafe_allow_html=True)
    with st.expander("🔬 Comparaison avancée — identifier la même usine sans code EMB", expanded=False):
        render_advanced_comparison(df)
